...
WayPoint 提供了三个维度的视图，协助你在不同场景下快速定位：

*   **🎯 焦点 (Focus - 智能引导):** 默认视图。基于智能算法，综合考虑**访问频率**、**最近访问**及**上下文预测**（如当前正在运行的开发工具），将你当下“最可能需要”的路径排在首位。
*   **⭐ 收藏 (Favorites - 手动固定):** 意愿驱动。存放你手动标记为收藏的路径。无论访问频率如何，这些路径始终固定在此，方便你随时访问核心目录。
*   **👣 足迹 (History - 时间线性):** 时间驱动。按照真实的访问先后顺序排列，是你回溯最近几分钟或几小时内工作轨迹的最佳方式。
